...
public sealed record Stopping(IzziQueue Queue) : ResourceState
{
    public override List<ResourceCommand> CommandsForQueue(IzziQueue queue) => Queue.User.Id == queue.User.Id
        ? [new ExecuteQueue()]
